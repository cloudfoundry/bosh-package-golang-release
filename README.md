...
$ bosh vendor-package golang-1.27-linux ~/workspace/bosh-package-golang-release
```
...
- `golang-1.27-{linux,darwin,windows}`: updated with latest version of go 1.27.x

To use `golang-*` package for compilation in your packaging script:

```bash
#!/bin/bash -eu
source /var/vcap/packages/golang-1.27-linux/bosh/compile.env
go build ...
```
or on Windows:

```powershell
. C:\var\vcap\packages\golang-1.27-windows\bosh\compile.ps1
go build ...
```
...
```bash
#!/bin/bash -eu
source /var/vcap/packages/golang-1.27-linux/bosh/runtime.env
go run ...
```
or on Windows:

```powershell
. C:\var\vcap\packages\golang-1.27-windows\bosh\runtime.ps1
go run ...
```
